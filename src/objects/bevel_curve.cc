...
void BeveledCurve::createSurface() {
  ...
  for (int i = 0; i < loops; i++) {
    ...
    Matrix cm1 = TranslateMatrix(p1.x, p1.y, p1.z) * getRotationMatrix(tan_1); 
    Matrix cm2 = TranslateMatrix(p2.x, p2.y, p2.z) * getRotationMatrix(tan_2); 
    ...
    for (int j = 0; j < turns; j++) {
      ...
      Vec a = cm1 * circ_1; Vec na = p1 - a;
      Vec b = cm1 * circ_2; Vec nb = p1 - b;

      Vec c = cm2 * circ_1; Vec nc = p2 - c;
      Vec d = cm2 * circ_2; Vec nd = p2 - d;
